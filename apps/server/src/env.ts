...
const envSchema = z.object({
  ...
  PLAID_AUTOMATIC_SYNC_CONCURRENCY: z.coerce.number().int().min(1).max(20).default(2),
  TELLER_APP_ID: z.string().optional().default(""),
  TELLER_ENV: z.enum(["sandbox", "development", "production"]).default("sandbox"),
  TELLER_CERT_FILE: z.string().optional().default(""),
  TELLER_KEY_FILE: z.string().optional().default(""),
  TELLER_SANDBOX_ACCESS_TOKEN: z.string().optional().default(""),
  TELLER_TRANSACTIONS_INITIAL_DAYS: z.coerce.number().int().min(1).max(730).default(90),
  TELLER_TRANSACTIONS_OVERLAP_DAYS: z.coerce.number().int().min(1).max(30).default(10),
  TELLER_WEBHOOK_SYNC_DEBOUNCE_SECONDS: z.coerce.number().int().min(0).max(3600).default(30),
  TELLER_AUTOMATIC_SYNC_CONCURRENCY: z.coerce.number().int().min(1).max(20).default(1),
  TELLER_WEBHOOK_SIGNING_SECRETS: z.string().default(""),
  TELLER_WEBHOOK_TOLERANCE_SECONDS: z.coerce.number().int().min(1).max(900).default(180),
  SIMPLEFIN_AUTOMATIC_SYNC_CONCURRENCY: z.coerce.number().int().min(1).max(20).default(2),
...
export const env = {
  ...
  tellerEnabled: Boolean(parsed.TELLER_APP_ID),
  tellerMtlsConfigured: Boolean(parsed.TELLER_CERT_FILE && parsed.TELLER_KEY_FILE),
